# PROMPT MAÎTRE V7 — TRANSFERT DE THREAD
...
THREAD → export → threads_to_process → ingest → extract → inject → DECISIONS / LESSONS → capital stratégique → data_cemetery

### Comportement ingest (v06+)

- Choix interactif au démarrage : [1] Batch production → threads_to_process/ / [2] Test pipeline → test_threads/
- Guard pré-ingest : vérification Notion avant ingest, alerte + confirmation si threads déjà traités (extraction_status=done ou injection_status=done) détectés
- Sur update page existante : statuts extraction_status / injection_status PRÉSERVÉS — ne jamais écraser un statut done
- Sur création nouvelle page : statuts initialisés à pending
- threads_to_process/ non versionné dans Git (.gitignore)
...
**Règle absolue : les trois numéros sont indépendants. README v06 + PROMPT v07 + CONTEXT v15 = configuration valide.**
...
- À terme : réseau d'agents spécialisés + L'Associé copilote décisionnel
