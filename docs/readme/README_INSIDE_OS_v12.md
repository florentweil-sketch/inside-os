...
Précédent : README_INSIDE_OS_v11
...
9. Push BACKLOG_DEV.md + BACKLOG_USER.md vers pages Notion miroir
10. Injection B99 sur `--inject`
...
data/
  ...
  threads_to_inject/    → file d'attente pour batch d'ingestion depuis cemetery
                          threads validés à injecter en mémoire
...
Les trois numéros sont indépendants. README v12 + PROMPT v12 + CONTEXT v25 = configuration valide.
...
| Agent Directeur des Achats | Politique achats groupe, négociations cadre, référencement prestataires, optimisation coûts — transverse F&A Capital + Atelier de la Colombe | B03 |
...
| Agent | Rôle |
|-------|------|
| **L'Associé** | Copilote décisionnel permanent — accès mémoire complète, peut être en désaccord, challenger une décision, jouer un rôle dans les relations externes. Prompt système fixe définissant son caractère et ses positions. |
...
| **Agent Classifieur Documents** | Routing automatique IA des documents métier entrants vers DB Notion cible et/ou dossier repo. Accès mémoire INSIDE OS. Bucket B09. |

### Deep probing inter-agents (V3)

Les agents peuvent s'interroger entre eux. Référence complète des agents, fiches de différenciation et règles de définition : voir PROMPT_ASSOCIE_vXX.
...
- `retry_count` max 2 retries auto sur inject_error — thread exclu de la boucle auto-pagination au-delà (retry_count >= 2), intervention manuelle requise
...
docs/
  ...
  prompts/                      → PROMPT_ASSOCIE_vXX.md
  pre-threads/                  → archive PRE_THREAD_B09-TXX.md (historique)
...
- Backup tar.gz automatique via `os-thread-close.mjs` (10 derniers conservés)
