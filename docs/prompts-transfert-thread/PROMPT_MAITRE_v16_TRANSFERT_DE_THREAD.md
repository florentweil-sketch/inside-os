# PROMPT MAÎTRE V16 — TRANSFERT DE THREAD
...
Précédent : PROMPT_MAITRE_v15 (B09-T39-Notion-Dev-027)
Évolution v15 → v16 : ajout de la règle de « Granularité des commits » (un geste cohérent = un commit, jamais de fourre-tout qui ment sur son contenu) + précision de l'étape 7 de la séquence de clôture. Déclenchée en B09-T40 par la décision de Florent : « autant de commits que nécessaire pour une photo propre du repo, toujours, ne plus demander ».
Évolution v14 → v15 : ajout de la « Règle anti-hallucination système » (section dédiée, ci-dessous). Déclenchée par l'hallucination du PRE_THREAD en B09-T39 (os:pre-thread affirmait « ✅ aucune divergence — système aligné » alors que le CONTEXT lu était v28 et non v30).
...
(os:pre-thread, os:close, vérifications d'intégrité) ET agents IA (L'Associé, agents métier).
...
## Note complémentaire — separation État / Doctrine (contexte de la règle)

La cause profonde de l'hallucination PRE_THREAD est structurelle : l'ÉTAT du système
(compteurs, versions, statuts) est aujourd'hui SAISI À LA MAIN dans plusieurs fichiers .md,
créant des copies divergentes qu'on tente de réconcilier par des scripts (eux-mêmes faillibles).

Direction architecturale gravée (BACKLOG_DEV SYSTEME P20) :
- **ÉTAT** (ce qui est) = calculé à la demande depuis une source unique (Notion live, git,
  filesystem). Jamais saisi à la main. Les .md l'AFFICHENT généré, avec timestamp + source.
- **DOCTRINE** (ce qui doit être) = PROMPT_MAITRE, PROMPT_ASSOCIE, README. Versionnée, peu de
  fichiers, éditée à la main sur décision.

Tant que l'État vit dans des .md édités à la main, la divergence est structurelle et la règle
anti-hallucination ci-dessus est une rustine sur une cause non traitée. La cible est de rendre
la divergence impossible, pas seulement détectable.
...
7. git commit — autant de commits que nécessaire pour une photo propre (voir règle granularité ci-dessous), jamais un fourre-tout
```
