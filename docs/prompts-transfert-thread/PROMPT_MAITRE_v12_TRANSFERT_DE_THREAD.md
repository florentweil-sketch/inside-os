# PROMPT MAÎTRE V12 — TRANSFERT DE THREAD
...
Précédent : PROMPT_MAITRE_v11 (B09-T33)
...
**Règle absolue test_threads/ :** ne jamais injecter un thread de test_threads/ dans thread_dump production. Les threads de test restent dans test_threads/ et ne passent jamais par le pipeline complet ingest+inject en production.
...
**Règle absolue : les trois numéros sont indépendants. README v10 + PROMPT v11 + CONTEXT v21 = configuration valide.**
...
**Règle archivage et versionnage — tous fichiers critiques (gravée B09-T37) :**
...
- Script canonique production : `notion-memory-server.mjs` (HTTP) — `notion-memory-chat.mjs` = test uniquement (Claude haiku-4-5, dépendance OpenAI supprimée)
