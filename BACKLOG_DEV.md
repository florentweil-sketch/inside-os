...
| P30 | Ouverture — Commercial (B05) sort vide : cause probable = absence de matière commerciale réelle dans le présent ; à re-diagnostiquer (un run avec log de troncature) APRÈS injection de dumps Commercial/Holding frais. Constaté sur 3 runs consécutifs malgré la règle de largeur (passe 1 garantie) et l'anti-monopole en post-traitement (`os/agents/ouverture/ouverture.mjs`, `enforceSourceDumpCap`) — les deux candidats B03 existants sont aussi B06 (donc classés Juridique), et le seul candidat B05 observé partageait un `source_dump_id` déjà saturé par Chantiers. Pas encore établi si c'est un vrai manque de matière ou un effet du plafonnage — diagnostic reporté après enrichissement du présent. | B09-T41 | [TODO] |
