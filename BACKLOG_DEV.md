...
| P34 | `claudeFetch()` (`os/lib/claude.mjs`) jette `stop_reason` par défaut — 7 appelants (`os/ingest/ingest-thread-dump.mjs` passe 1 + passe 2, `os/extract/extract-thread-dump.mjs`, agents Synthèse/Associé/Ouverture/Pilotage) ne détectent donc pas une réponse tronquée par `max_tokens` : le contenu partiel revient comme s'il était complet. Corrigé pour un seul appelant en B09-T43 (Agent Ingestion Docs, `commit 73aa307`) — `claudeFetch()` expose désormais `{ text, stopReason }` via un paramètre optionnel `full=true`, rétro-compatible. Les 7 autres appelants restent non couverts — cousin de P13/P23 (famille des sorties silencieuses/tronquées). Pas de bascule massive faite : chaque appelant a un contexte différent (JSON strict passe 1/2, texte libre agents) qui mérite un traitement d'erreur adapté au cas par cas, pas un copier-coller mécanique. | B09-T43 | [TODO] |
