...
Derniere mise a jour : 2026-08-15 (B09-T42)
Version : v07
...
| P31 | **Point ouvert non résolu (B09-T42)** — `os/ingest/ingest-thread-dump.mjs` exclut encore par défaut le bucket B09 (`DEFAULT_SKIP_BUCKETS=["B09"]`), reste du protocole de clôture abandonné (`os-thread-close.mjs` retiré et archivé en B09-T42). La nouvelle doctrine (CLAUDE.md, README.md, PROMPT_MAITRE v17) documente qu'un thread B09 se clôture désormais par le même flux dump → pipeline que tout thread, mais le code n'a pas été changé — un `os:ingest` batch sans `--only` exclut toujours silencieusement les dumps B09 déposés dans `threads_to_process/`. À trancher : retirer l'exclusion par défaut (B09 traité comme tout bucket), ou documenter une raison de la garder et exiger `--only` explicite pour B09 en attendant. | B09-T42 | [TODO] |
